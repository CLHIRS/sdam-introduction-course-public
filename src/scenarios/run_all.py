...
def _ensure_src_on_path() -> None:
    """Ensure `import scenarios.*` works when running as a script.

    When executed as `python src/scenarios/run_all.py`, Python's default sys.path
    does NOT include the repo's `src/` folder, so `import scenarios` fails.

    In normal usage (after `poetry install`), this is a no-op.
    """
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument(
        "--fast",
        action="store_true",
        help="Run a reduced-workload version of scenarios (intended for smoke tests).",
    )
    p.add_argument(
        "--only",
        action="append",
        default=[],
        help="Run only a specific scenario (repeatable): module path or display name",
    )
    p.add_argument(
        "--skip",
        action="append",
        default=[],
        help="Skip a scenario (repeatable): module path or display name",
    )
    ...
    run_all(
        des_seed=int(args.des_seed),
        crn_seed0=int(args.crn_seed0),
        only=list(args.only),
        skip=list(args.skip),
        run_env=(not bool(args.no_env)),
    )
    return 0
