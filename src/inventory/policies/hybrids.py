...
class HybridDlaRolloutBasePfaPolicy(Policy):
    ...
    def __repr__(self) -> str:
        base_policy_name = type(self.base_policy).__name__ if hasattr(self.base_policy, "__class__") else None
        if hasattr(self.base_policy, "act"):
            base_policy_name = type(self.base_policy).__name__
        else:
            base_policy_name = getattr(self.base_policy, "__name__", type(self.base_policy).__name__)
        return (
            "HybridDlaRolloutBasePfaPolicy("
            f"base_policy={base_policy_name!r}, H={self.H}, n_rollouts={self.n_rollouts}, "
            f"dx={self.dx}, x_max={self.x_max}, decision_seed={self.decision_seed}, "
    ...
    def _candidate_radius_for_state(self, state: State, t: int, info: dict) -> int:
    ...
    def _candidate_actions(self, state: State, t: int, info: dict) -> np.ndarray:
        ...
        if self.include_full_action_grid or radius_steps == 0:
            return self._A.copy()
