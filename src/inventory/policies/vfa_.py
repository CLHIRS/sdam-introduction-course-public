...
class PostDecisionGreedyVfaPolicy(Policy):
    ...
    def __post_init__(self) -> None:
        # Core control knobs (discount + discrete action grid + TD step size).
        self.gamma = float(self.gamma)
        self.x_max = float(self.x_max)
        self.dx = int(self.dx)
        self.alpha = float(self.alpha)

        # Monte Carlo expectation settings used inside act().
        self.expectation_samples = int(self.expectation_samples)
        self.expectation_seed = int(self.expectation_seed)

        # Post-decision value-function feature configuration.
        ...
        # Linear value-function weights for V_hat(S_post, t) = w^T phi(S_post, t).
    ...
    def _phi_post(self, S_post: np.ndarray, t: int) -> np.ndarray:
        # Normalized post-decision coordinates.
        # S_post is inventory-after-order, before-demand.
        inv = float(S_post[0]) / self.inv_scale
        tau = float(t) / 50.0

        if self.feature == "linear":
            # Linear basis for V_hat.
            base = [1.0, inv, tau]
        else:
            # Quadratic basis with interaction terms.
    ...
    def _ensure_w(self, phi: np.ndarray) -> None:
        if self.w is None:
            # Lazy initialization keeps API simple and dimension-consistent with features.
    ...
    def vbar_hat(self, S_post: np.ndarray, t: int) -> float:
        # Learned post-decision value approximation:
        #   V_hat(S_post, t) = w^T phi(S_post, t)
    ...
    def _candidate_orders(self) -> np.ndarray:
        ...
        # Discrete action set X = {0, dx, 2dx, ..., x_max}.
    ...
    def _seed_for_expectation(self, t: int, inv: float) -> int:
        ...
        # is repeatable. This makes act() deterministic for fixed inputs.
    ...
    def _expected_one_step_cost(self, S: np.ndarray, x: float, t: int) -> float:
        # Monte Carlo estimator of E[C(S_t, x, W_{t+1}, t)] in the objective:
        #   (1/n) * sum_i C(S_t, x, W_{t+1}^{(i)}, t)
        ...
        if isinstance(exo, ExogenousPoissonSeasonal):
            # Seasonal Poisson demand: D_{t+1} ~ Poisson(lambda_t(t)).
            lam = exo.lambda_t(t)
        ...
        if isinstance(exo, ExogenousPoissonRegime):
            # Regime model: sample R_{t+1} from P[R_t, .], then demand from
            # regime-specific Poisson(lambda_{R_{t+1}}).
        ...
        costs = np.empty(n, dtype=float)
        for i in range(n):
            # Generic exogenous model path.
            W = exo.sample(S, X, t, rng)
            costs[i] = float(self.model.cost_func(S, X, W, t))
        return float(costs.mean())

    def act(self, state: State, t: int, info: Optional[dict] = None) -> Action:
        # Greedy post-decision VFA action rule:
        #   x_t in argmin_x [ E[C(S_t, x, W_{t+1}, t)] + gamma * V_hat(S_t^x, t) ]
    ...
    def train_td_value(
        ...
        *,
        ...
        # TD(0) training loop on the post-decision value approximation.
        # Update form:
        #   delta_t = C_t + gamma * V_hat(S_{t+1}^{x_next}, t+1) - V_hat(S_t^{x_t}, t)
        #   w <- w + alpha * delta_t * phi(S_t^{x_t}, t)
        T = int(T)
        ...
        for ep_seed in ep_seeds:
            # CRN step seeds make episode rollouts reproducible.
            step_seeds = self.model.sample_crn_step_seeds(ep_seed, T)
            S_t = np.asarray(S0, dtype=float).copy()

            for t in range(T):
                if rng.random() < epsilon:
                    # Exploration step (epsilon-greedy behavior policy).
                    x = float(rng.choice(self._candidate_orders()))
                else:
                    # Exploitation under current greedy policy.
                ...
                if t == T - 1:
                    # Terminal bootstrap value is zero.
                    v_next = 0.0
                else:
                    # Greedy bootstrap action for next state.
...
def _build_supervised_regressor(
    *,
    mode: str,
    ridge_alpha: float,
    elastic_alpha: float,
    ...
    """Shared regression backend builder for fitted VFA variants."""
    ...
    if mode == "ridge":
        try:
            ...
            return _RidgeFallback(alpha=ridge_alpha)
    ...
    raise ValueError("mode must be one of {'ridge','elastic','decision_tree','tree','gbrt','mlp'}")


@dataclass
class PostDecisionFittedGreedyVfaPolicy(Policy):
    """Batch-fitted post-decision VFA with greedy action selection.

    The policy still acts greedily:

      x_t = argmin_x E[C(S_t, x, W_{t+1})] + gamma * Vbar_hat(S_t^x, t)

    but the post-decision value approximation is trained by repeated supervised
    regression on Bellman-style targets rather than online TD updates.
    """
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"expectation_samples={self.expectation_samples}, "
            f"expectation_seed={self.expectation_seed})"
        )
    ...
    def _predict_v(self, S_post: np.ndarray, t: int) -> float:
        if self.v_model is None:
            return 0.0
        phi = self._phi_post(S_post, t).reshape(1, -1)
        return float(np.asarray(self.v_model.predict(phi), dtype=float).reshape(-1)[0])

    def vbar_hat(self, S_post: np.ndarray, t: int) -> float:
        return self._predict_v(S_post, t)
        ...
        *,
        ...
        for ep_seed in ep_seeds:
            ...
            for t in range(T):
                ...
                X_t = np.array([float(int(np.round(x)))], dtype=float)
                S_post_t = self._make_post_state(S_t, x)
                ...
                Spost_list.append(np.asarray(S_post_t, dtype=float).copy())
                t_list.append(int(t))
        ...
        Phi = np.vstack([self._phi_post(Spost_list[i], t_list[i]) for i in range(len(Spost_list))])
        c_arr = np.asarray(c_list, dtype=float)

        for _k in range(n_iterations):
            y = np.empty_like(c_arr)
            for i in range(len(Spost_list)):
                if t_list[i] >= T - 1:
                    y[i] = c_arr[i]
                else:
                    x_next = float(self.act(Snext_list[i], tnext_list[i])[0])
                    S_post_next = self._make_post_state(Snext_list[i], x_next)
                    y[i] = c_arr[i] + self.gamma * self.vbar_hat(S_post_next, tnext_list[i])

            model_k = self._fit_model()
            model_k.fit(Phi, y)
            self.v_model = model_k
...
class FqiGreedyVfaPolicy(Policy):
    ...
    def __post_init__(self) -> None:
        # Core control knobs (discount + discrete action grid bounds).
        self.gamma = float(self.gamma)
        self.x_max = float(self.x_max)
        self.dx = int(self.dx)

        # Regression model / featurization hyperparameters.
        self.mode = str(self.mode)
        self.ridge_alpha = float(self.ridge_alpha)
        self.elastic_alpha = float(self.elastic_alpha)
        self.elastic_l1_ratio = float(self.elastic_l1_ratio)
        ...
        # q_model stores Q_hat after training. If None, act() falls back to zeros.
        self.q_model = None
    ...
    def _phi_sa(self, S: np.ndarray, t: int, x: float) -> np.ndarray:
        # Normalized coordinates used by the feature map phi(S, t, x).
        # This mirrors the notation in lectures/L06_VFA/FqiGreedyVfaPolicy.md.
        ...
        if self.feature == "linear":
            ...
            # Quadratic (poly2) basis with interactions.
    ...
    def _fit_model(self):
        # Function class F used in the projected Bellman backup.
        # FQI target construction is independent of this choice.
        return _build_supervised_regressor(
            mode=self.mode,
            ridge_alpha=self.ridge_alpha,
            elastic_alpha=self.elastic_alpha,
    ...
    def _predict_q(self, S: np.ndarray, t: int, xs: np.ndarray) -> np.ndarray:
        if self.q_model is None:
            # Before training, Q_hat is taken as zero for all actions.
    ...
    def _min_q_next(self, S_next: np.ndarray, t_next: int) -> float:
        # Computes min_{x' in X} Q_hat(S_next, t_next, x'),
        # the bootstrap term in Bellman targets.
        xs = self._candidate_orders()
    ...
    def act(self, state: State, t: int, info: Optional[dict] = None) -> Action:
        # Greedy action rule:
        #   x_t in argmin_{x in X} Q_hat(S_t, t, x)
        xs = self._candidate_orders()
    ...
    def train_fqi(
        ...
        *,
        ...
        # -----------------------------------------------------
        # Stage 1: collect an offline transition dataset D
        #   (S_t, t, a_t, C_t, S_{t+1}, t+1)
        # -----------------------------------------------------
        for ep_seed in ep_seeds:
            step_seeds = self.model.sample_crn_step_seeds(ep_seed, T)
            S_t = np.asarray(S0, dtype=float).copy()
            ...
            for t in range(T):
                if behavior == "random":
                    # Pure behavior policy for broad coverage.
                    x = float(rng.choice(xs))
                elif behavior == "egreedy":
                    # epsilon-greedy behavior w.r.t. current Q_hat.
                    if rng.random() < eps_behavior:
                        x = float(rng.choice(xs))
                    else:
                        x = float(self.act(S_t, t)[0])
        ...
        # Features for all visited (S_t, t, a_t). Reused across FQI iterations.
        Phi = np.vstack([self._phi_sa(S_list[i], t_list[i], a_list[i]) for i in range(len(S_list))])
        c_arr = np.asarray(c_list, dtype=float)

        # -------------------------------------------------
        # Stage 2: fitted Q iteration (projected backups)
        #   y_i = c_i + gamma * min_{a'} Q_hat(S'_i, t_i+1, a')
        # then fit q_model on (Phi, y)
        # -------------------------------------------------
        for _k in range(n_iterations):
            y = np.empty_like(c_arr)
            for i in range(len(S_list)):
                if t_list[i] >= T - 1:
                    # Terminal sample: no continuation term.
                    y[i] = c_arr[i]
                else:
                    # Bellman backup target (cost-minimization form).
                    y[i] = c_arr[i] + self.gamma * self._min_q_next(Snext_list[i], tnext_list[i])

            model_k = self._fit_model()
            model_k.fit(Phi, y)
            # Q_hat_k becomes the bootstrap model for the next iteration.
            self.q_model = model_k


# Backwards-compatible names from the notebook
PostDecision_Greedy_VFA = PostDecisionGreedyVfaPolicy
PostDecision_Fitted_Greedy_VFA = PostDecisionFittedGreedyVfaPolicy
...
__all__ = [
    ...
    "PostDecision_Fitted_Greedy_VFA",
    "FQI_Greedy_VFA",
]
